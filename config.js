...
function getConfig(env) {
    ...
    case 'localnet':
        config = {
            ...
            keyPath:`${process.env.NEAR_CLI_LOCALNET_KEY_PATH_ROOT}/validator_key.json` || `${process.env.HOME}/.near/validator_key.json`,
        //    keyPath: getKeyPath(process.options),
...
function getKeyPath(options)
{
    const args = process.argv.slice(2); // Skip the first two elements ('node' and the script name)

    let nodeUrl;

    // Loop through the arguments
    args.forEach(arg => {
        if (arg.startsWith('--nodeUrl=')) {
            nodeUrl = arg.split('=')[1]; // Get the value after '='
        }
    });
